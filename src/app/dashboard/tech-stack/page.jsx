  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        await addDoc(collection(db, "tech_stack"), formData);
      }
      setIsAdding(false);
      setEditingId(null);
      ...
      alert("Failed to save tech stack item. Check console for details.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      <header className="flex justify-between items-center border-b border-white/5 pb-8">
        <div>
          <h1 className="text-2xl font-bold uppercase tracking-[0.2em]">Tech Stack</h1>
      ...
      {isAdding && (
        <div className="bg-gray-900/50 border border-cyan-500/30 p-8 space-y-6 animate-in fade-in slide-in-from-top-4 duration-500 rounded-lg">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-6">
            ...
            <Input label="Icon Name (e.g. SiReact)" name="icon" value={formData.icon} onChange={handleInputChange} className="col-span-2" required />
            <button 
              type="submit" 
              disabled={loading}
      ...
      {loading && !items.length ? (
        ...
        <div className="py-20 text-center border border-dashed border-white/10 rounded-lg">
          <p className="text-gray-500 text-[10px] uppercase tracking-[0.3em]">No tech stack items found</p>
        </div>
      ) : (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/10 bg-white/5">
                <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-cyan-400">Name</th>
                <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-cyan-400">Category</th>
            ...
            <tbody className="divide-y divide-white/5">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="px-6 py-4 text-[10px] text-gray-400 uppercase">{item.category}</td>
